# Stores CRUD Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Status: IN PROGRESS**

**Goal:** Build a full CRUD management page for stores inside the dashboard, accessible only to super admins, following the existing MVC + Service architecture established by the Users CRUD.

**Architecture:** Middleware -> Controller -> Service -> Model. All API routes prefixed with `/dashboard`. Vue Router handles `/dashboard/stores` client-side within the existing dashboard SPA. Super admin only access — both sidebar visibility and API authorization.

**Tech Stack:** Laravel 13, PHPUnit 12 (SQLite in-memory), Vue 3 Composition API, Tailwind CSS v4, axios, Vue Router 5.

---

## Task 1: Migration — add `deleted_at` to stores

**Files:**
- Create: `database/migrations/2026_08_18_000001_add_soft_deletes_to_stores_table.php`
...
Expected: migration runs without error, `deleted_at` column added to `stores`.

**Step 3: Commit**

```bash
git add database/migrations/2026_08_18_000001_add_soft_deletes_to_stores_table.php
git commit -m "feat(stores): add soft deletes migration for stores table"
```

---

## Task 2: Update Store model — add SoftDeletes trait

**Files:**
- Modify: `app/Models/Store.php`

**Step 1: Add `SoftDeletes` trait to the Store model**
...
git commit -m "feat(stores): add SoftDeletes trait to Store model"
```

---

## Task 3: Validation middleware — `ValidateStoreRequest`

**Files:**
- Create: `app/Http/Middlewares/Stores/ValidateStoreRequest.php`
...
```bash
git add app/Http/Middlewares/Stores/ValidateStoreRequest.php
git commit -m "feat(stores): add ValidateStoreRequest middleware"
```

---

## Task 4: Service — `StoreService`

**Files:**
- Create: `app/Services/Stores/StoreService.php`
...
```bash
git add app/Services/Stores/StoreService.php
git commit -m "feat(stores): add StoreService with CRUD logic"
```

---

## Task 5: Controller — `StoreController`

**Files:**
- Create: `app/Http/Controllers/Stores/StoreController.php`
...
```bash
git add app/Http/Controllers/Stores/StoreController.php
git commit -m "feat(stores): add StoreController"
```

---

## Task 6: API routes — register store CRUD endpoints

**Files:**
- Modify: `routes/api.php`

**Step 1: Add store routes inside the existing `RedirectIfNotAuthenticated` group**
...
Add these routes inside the existing `Route::middleware(RedirectIfNotAuthenticated::class)->group(function () { ... })` block, after the user routes:
...
**IMPORTANT**: The GET route uses `/dashboard/stores/list` (not `/dashboard/stores`) because `/dashboard/stores` already exists and is used by `UserController@stores` to return the store dropdown list for the users page. The new listing endpoint returns paginated data with search/filter support.

**Step 2: Commit**

```bash
git add routes/api.php
git commit -m "feat(stores): register store CRUD API routes"
```

---

## Task 7: Vue route — register `/dashboard/stores`
...
Add the route entry before the catch-all `/:pathMatch(.*)*`:
...
git commit -m "feat(stores): add /dashboard/stores Vue route"
```

---

## Task 8: Sidebar — add Stores link (super admin only)

**Files:**
- Modify: `resources/js/views/Common/LeftNavigationBar.vue`

**Step 1: Make the sidebar aware of the auth user role**

The sidebar currently has a static `navItems` array. We need to:
1. Accept the auth user role as a prop (or fetch it)
2. Filter the "Stores" link to only show for super admins

Since `DashboardLayout.vue` wraps the sidebar, we need to check how auth user info flows. The simplest approach: accept a prop from the parent, or fetch the auth user in the sidebar itself.

Looking at the existing code, the sidebar has no props and no auth awareness. The cleanest approach is to fetch the auth user in the sidebar and compute the filtered nav items.
...
const allNavItems = [
    { label: 'Stores',   href: '/dashboard/stores',   icon: 'M16 11V7a4 4 0 00-8 0v4M5 9h14l1 11H4L5 9z', superAdminOnly: true },
...
const navItems = computed(() => {
    const isSuperAdmin = authUser.value?.role === 'super_admin';
...
git commit -m "feat(stores): show Stores sidebar link for super admins only"
```

---

## Task 9: Vue component — `Stores.vue`

**Files:**
- Create: `resources/js/views/Stores/Stores.vue`

**Step 1: Create the Stores CRUD component**

This mirrors `Users.vue` exactly in structure but adapted for store fields. The component includes:
- Success toast (3s auto-dismiss)
- Page header with "Add Store" button
- Filters panel: search (store_name, email, address) + status filter (active/inactive/all)
- DataTable with columns: Store (name+email), Address, Owner, Mobile, Status, Created, Updated, Actions
- Create/Edit modal with all store fields
...
<template>
    <DashboardLayout>
        ...
        <DataTable
            ...
            :empty-subtitle="search ? 'Try a different search term.' : 'Add your first store to get started.'"
            item-label="store"
...
const hasActiveFilters = computed(() => search.value !== '' || filterStatus.value !== 'active');
...
```bash
git add resources/js/views/Stores/Stores.vue
git commit -m "feat(stores): add Stores.vue CRUD component"
```
...
**Created:**
1. `database/migrations/2026_08_18_000001_add_soft_deletes_to_stores_table.php`
2. `app/Http/Middlewares/Stores/ValidateStoreRequest.php`
3. `app/Services/Stores/StoreService.php`
4. `app/Http/Controllers/Stores/StoreController.php`
5. `resources/js/views/Stores/Stores.vue`

**Modified:**
1. `app/Models/Store.php` — added `SoftDeletes` trait
2. `routes/api.php` — added store CRUD routes
3. `resources/js/router/index.js` — added `/dashboard/stores` route
4. `resources/js/views/Common/LeftNavigationBar.vue` — super admin conditional Stores link
